...
void FileStore::_do_op(OpSequencer *osr, ThreadPool::TPHandle &handle)
{
  ...
  osr->apply_lock.Lock();
  Op *o = osr->peek_queue(); // get an Op

  // seq asserts, because here we are not replaying so we are paused when filestore
  // sync thread is updating its committing_seq, so deadlock is impossible
  // for replaying, refer to JournalingObjectStore::journal_replay
...
int FileStore::_do_transactions(
  ...
  uint64_t op_seq, // seq of the transaction list, generated by SubmitManager
  ThreadPool::TPHandle *handle)
{
  int r = 0;
  int trans_num = 0; // transaction seq local to this transaction list

  for (list<Transaction*>::iterator p = tls.begin();
       p != tls.end();
       ++p, trans_num++) {
    // do individual transaction
  ...
  ThreadPool::TPHandle *handle)
{
  ...
  // Transaction::Op is a filesystem modification operation, it's not the same 
  // as FileStore::Op which contains a list of Transaction(s)
                     ...
                     const bufferlist& bl, uint32_t fadvise_flags)
{
  ...
  // flush?
  if (!replaying &&
      g_conf->filestore_wbthrottle_enable) // default is true
