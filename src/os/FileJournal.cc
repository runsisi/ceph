...
int FileJournal::prepare_multi_write(bufferlist& bl, uint64_t& orig_ops, uint64_t& orig_bytes)
{
  ...
  int eleft = g_conf->journal_max_write_entries; // default is 100
  unsigned bmax = g_conf->journal_max_write_bytes; // default is 10 << 20, i.e. 10M

  if (full_state != FULL_NOTFULL) // journal is full
    return -ENOSPC;
  
  while (!writeq_empty()) { // the writeq is not empty
    // if there is space to write an encoded write_item, then encode it, 
    // else return immediately
    int r = prepare_single_write(bl, queue_pos, orig_ops, orig_bytes);
    if (r == -ENOSPC) {
      // no space to write an encoded write_item

      if (orig_ops) // we have encoded a write_item at least
	break;         // commit what we have

      // no space to write an encoded write_item, and we have not encoded 
      // a write_item ever

      if (logger)
	logger->inc(l_os_j_full);

      if (wait_on_full) { // if journal is write ahead mode, then FileStore::mount will set this to true
	...
	while (!writeq_empty()) { // delete all write_item(s) in the writeq
    ...
    // check if we can encode more write_item
...
void FileJournal::queue_completions_thru(uint64_t seq)
{
  ...
  while (!completions_empty()) {
    ...
    // we have written a write_item to journal, queue the callback
...
int FileJournal::prepare_single_write(bufferlist& bl, off64_t& queue_pos, uint64_t& orig_ops, uint64_t& orig_bytes)
{
  ...
  uint64_t seq = next_write.seq; // this seq is a seq generated by SubmitManager to identify an instance of FileStore::Op
  ...
  writing_seq = seq; // the max seq of the journal entry we are about to write
...
void FileJournal::do_write(bufferlist& bl)
{
  ...
  if (!directio) {
    ...
    if (g_conf->filestore_fadvise) // default is true
...

  {
    ...
    // writing_seq is updated in prepare_single_write which means the max 
    // seq of the journal entry we are about to write
    journaled_seq = writing_seq; // the max seq we have journaled
...
// journal the bufferlist of a transaction list from writeq
  ...
  while (1) {
    {
      ...
      if (writeq.empty() && !must_write_header) {
        // no write_item in the writeq and we do not have to write header, so
        // we just wait
	if (write_stop)
	  break;
    ...
    // ok, we are signalled, something has to write
    ...
    bufferlist bl;
    // encode write_item(s) from writeq into bl
    ...
    if (r == -ENOSPC) { // has not even encoded a write_item yet
      if (write_stop) {
	...
	dout(20) << "write_thread_entry full, going to sleep (waiting for commit)" << dendl;
        // wait until we are stopping or FileJournal::committed_thru signals us
	commit_cond.Wait(write_lock);
	dout(20) << "write_thread_entry woke up" << dendl;
	continue; // restart to encode write_item
      }
    }
    assert(r == 0);

    // ok, we have encoded a write_item at least
    ...
    if (aio)
      ...
      do_write(bl);
#else
    // write the encoded write_item(s)
    do_write(bl);
#endif
