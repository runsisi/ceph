...
private:
 ...
 public:
  FileJournal(uuid_d fsid, Finisher *fin, Cond *sync_cond, const char *f, bool dio=false, bool ai=true, bool faio=false) :
    ...
    fd(-1),
    writing_seq(0),
    throttle_ops(g_ceph_context, "journal_ops", g_conf->journal_queue_max_ops), // default is 300
    throttle_bytes(g_ceph_context, "journal_bytes", g_conf->journal_queue_max_bytes), // default is 32 << 20, i.e. 32M
