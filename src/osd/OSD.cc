...
// called by
// Messenger::ms_deliver_get_authorizer <-
//      AsyncConnection::_process_connection
//      AsyncConnection::handle_connect_reply
bool OSD::ms_get_authorizer(int dest_type, AuthAuthorizer **authorizer, bool force_new)
{
  ...
  if (force_new) {
    /* the MonClient checks keys every tick(), so we should just wait for that cycle
       to get through */
    if (monc->wait_auth_rotating(10) < 0) { // wait MonClient::tick update rotating keys
  ...
  *authorizer = monc->build_authorizer(dest_type); // dest_type is service id in cephx
