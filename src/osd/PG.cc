...
// called by
// PG::take_waiters, which called by state machine reacts ActMap
void PG::take_op_map_waiters()
{
  Mutex::Locker l(map_lock);

  // was inserted by PG::queue_op when the map is older than the peer
  for (list<OpRequestRef>::iterator i = waiting_for_map.begin();
    ...
    if (op_must_wait_for_map(get_osdmap_with_maplock()->get_epoch(), *i)) {
      ...
      // will be processed by OSD::ShardedOpWQ::_process
...
void PG::queue_op(OpRequestRef& op)
{
  ...
  if (!waiting_for_map.empty()) {
    ...
    // will be queued on OSDService::op_wq by PG::take_op_map_waiters
  ...
  // OSD::ShardedOpWQ::_enqueue, will be processed by OSD::ShardedOpWQ::_process
...
// called by
// PG::take_op_map_waiters
// PG::queue_op
// PrimaryLogPG::do_request, for assertion only
...
void PG::take_waiters()
{
  ...
  // try to requeue ops on PG::waiting_for_map that has been blocked
  // in PG::queue_op bcoz of the laggy map onto OSDService::op_wq
  take_op_map_waiters();
...
// RemoteReservationRejected was queued by RepNotRecovering::react(RequestBackfillPrio),
// or RepWaitBackfillReserved::react(RemoteBackfillReserved) when osd is full
boost::statechart::result
...
boost::statechart::result PG::RecoveryState::WaitUpThru::react(const ActMap& am)
{
  ...
  if (!pg->need_up_thru) {
    // WaitUpThru is substate of Peering, so boost::statechart::transition< Activate, Active >
    // and PG::RecoveryState::Active::Active will call pg->activate
    post_event(Activate(pg->get_osdmap()->get_epoch()));
  }
