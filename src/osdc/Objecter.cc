...
void Objecter::_send_linger(LingerOp *info)
{
  ...
  // info->is_watch is set to true in Objecter::linger_watch which always
  // calls to here or this linger op is being resend, so info->is_watch
  // is always true after librados::IoCtxImpl::watch is called
  if (info->registered && info->is_watch) {
    // we have already registered this linger op in cluster, so this 
    // _send_linger call should be a resend operation
    ...
    // the first time to send this linger op
    ldout(cct, 15) << "send_linger " << info->linger_id << " register" << dendl;
    opv = info->ops;
    // info->registered will be set when this callback called
    C_Linger_Commit *c = new C_Linger_Commit(this, info);
    if (!info->is_watch) {
      info->notify_id = 0;      // notify_id is set when C_Linger_Commit finished, this id is generated by the cluster
      poutbl = &c->outbl;
    }
    oncommit = c;
  }
  ...
  // linger op always changes to a regular Op when sending to cluster
  Op *o = new Op(info->target.base_oid, info->target.base_oloc,
		 opv, info->target.flags | CEPH_OSD_FLAG_READ,
		 NULL, NULL,
		 info->pobjver);
  o->oncommit_sync = oncommit;  // when this Op finished, call this callback
  o->outbl = poutbl;
  o->snapid = info->snap;
  o->snapc = info->snapc;
  o->mtime = info->mtime;
  ...
  // this tid id monotonically increase in this Objecter, every Op will
  // identified by a tid
  o->tid = last_tid.inc();
...
void Objecter::linger_cancel(LingerOp *info)
{
  ...
  // remove from OSDSession::linger_ops and 
  // Objecter::linger_ops, Objecter::linger_ops_set
  _linger_cancel(info);
  info->put();
}
...
void Objecter::_linger_cancel(LingerOp *info)
{
  ...
  if (!info->canceled) {
    ...
    s->lock.get_write();
    // remove from OSDSession::linger_ops
				  ...
				  version_t *objver)
{
  ...
  RWLock::WLocker wl(rwlock);
  // calc target osd to send, register this linger op in session then send it
  _linger_submit(info);
...
void Objecter::_linger_submit(LingerOp *info)
{
  ...
  assert(info->linger_id);      // when we allocate a linger op, we set this field
  ...
  // if we have got a session already with the primary osd, then reuse it, else
  // we allocate a new session and setup a network connection to target osd
  int r = _get_session(info->target.osd, &s, lc);
  assert(r == 0);
  s->lock.get_write();
  // register this linger op to session
...
ceph_tid_t Objecter::_op_submit_with_budget(Op *op, RWLock::Context& lc, int *ctx_budget)
{
  ...
  if (!op->ctx_budgeted || (ctx_budget && (*ctx_budget == -1))) {
    // may sleep while wating for budget
  ...
  // osd_timeout and mon_timeout are initialized in Objecter's ctor and
  // default to 0, which means never timeout
  if (osd_timeout > 0) {
    if (op->tid == 0)
