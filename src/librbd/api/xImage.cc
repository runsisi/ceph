...
private:
  ...
  // [in]
  const librbdx::image_info_t* m_info = nullptr;
...
//    auto ptr = std::make_shared<InfoRequest>(std::forward<Args>(args)...);
    ptr->init();
    return ptr;
  }
...
private:
  ...
  void handle_get_info(int r) {
    ...
    // image du will be populated later
    ...
    for (auto& it : cls_snaps) {
      ...
      if (m_flags & static_cast<uint64_t>(librbdx::info_filter_t::INFO_F_CHILDREN_V1)) {
        // clone v1
  ...
  void get_dus() {
    ...
    if (!(m_flags & (static_cast<uint64_t>(librbdx::info_filter_t::INFO_F_IMAGE_DU) |
        static_cast<uint64_t>(librbdx::info_filter_t::INFO_F_SNAP_DU)))) {
      complete(0);
      return;
    }
    ...
    if (m_flags & static_cast<uint64_t>(librbdx::info_filter_t::INFO_F_SNAP_DU)) {
...
int xImage<I>::get_info(librados::IoCtx& ioctx,
  ...
  if (flags & static_cast<uint64_t>(librbdx::info_filter_t::INFO_F_CHILDREN_V1)) {
  ...
  auto req = InfoRequest<I>::create(ioctx, on_finish, image_id, children,
      info, flags);
  req->send();
...
int xImage<I>::list_info(librados::IoCtx& ioctx,
  ...
  for (const auto& image : images) {
    ...
    auto req = InfoRequest<I>::create(ioctx, on_finish, id, children,
        &info, flags);
    throttle.start_op();
