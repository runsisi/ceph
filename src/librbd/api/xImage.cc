...
private:
  ...
  int m_pending_count;
  int m_r;
  std::map<uint64_t, uint64_t>* m_du;
};
...
public:
  ThrottledDuRequest_v2(librados::IoCtx& io_ctx, SimpleThrottle& throttle,
      const std::string& image_id, const std::string& image_name,
      std::map<uint64_t, uint64_t>* du, int* r)
    : m_cct(reinterpret_cast<CephContext*>(io_ctx.cct())),
      m_throttle(throttle),
      m_image_id(image_id), m_image_name(image_name),
      m_r(r) {
...
int xImage<I>::get_size(librados::IoCtx& io_ctx,
    const std::string& id, uint64_t snap_id, xSizeInfo* info) {
...
//  int r = xTrash<I>::list(io_ctx, &trashes);
//  if (r < 0 && r != -EOPNOTSUPP) {
//    return r;
//  }
//  for (auto& it : trashes) {
