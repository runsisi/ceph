...
void ImageWatcher<I>::schedule_request_lock(bool use_timer, int timer_delay) {
  ...
  if (this->is_registered(this->m_watch_lock)) {
    ...
    if (use_timer) {
      if (timer_delay < 0) {
        timer_delay = RETRY_DELAY_SECONDS; // 1.0
      }
...
void ImageWatcher<I>::handle_request_lock(int r) {
  ...
  if (r == -ETIMEDOUT) {
    ...
    ldout(m_image_ctx.cct, 15) << this << " will retry in " << retry_timeout
                               << " seconds" << dendl;
    // will be canceled by ImageWatcher<I>::handle_payload(const ReleasedLockPayload)
    // if the lock is released by the current lock owner
